...
#                weight *= functools.reduce(lambda x,y: x*y, (G.max_probability[a][0] for a in args))
    ...
    def query(self, S,t):
        ...
        a_f = self.G.arities[f] # arities (symbol arguments of f)
        ...
        for i in range(len(a_f)):
            ...
            sub_term = new_term.arguments[i]
            succ_sub_term = self.query(a_f[i], arg[i]) # if -1 this subterm has no successor, so isinstance(succ_sub_term,Program) is false in this case

            if isinstance(succ_sub_term, Program):
                new_term.arguments[i] = succ_sub_term
                hash_new = hash_term(new_term)
