...
def ecIterator(grammar, tasks,
	...
	for j in range(resume or 0, iterations):
		...
		"""new code for pickling grammars"""
		
		recognizer = result.recognitionModel
		if recognizer is not None:
			grammars = {task: recognizer.grammarOfTask(task)
						for task in testingTasks + tasks}
			...
			grammars = (result.grammars[-1],grammars)
			print("dump will include neural network grammars")
		else:
			grammars = result.grammars[-1]
			print("dump will not include neural network grammars")
		fn = "/tmp/all_grammars.pickle"
		with open(fn,"wb") as handle:
			pickle.dump(grammars, handle)
		print("dumped all of the grammars to",fn)
		assert False
